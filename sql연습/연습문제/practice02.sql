...
-- 예) 2014년 07월 10일
select emp_no, date_format(min(hire_date),'%Y년 %m월 %d일') as '입사일'
from employees;
...
select date_format(now(), '%Y') - date_format(max(birth_date),'%Y') as '어린사원',
date_format(now(), '%Y') - date_format(min(birth_date),'%Y') as '최연장자사원'
from employees;
