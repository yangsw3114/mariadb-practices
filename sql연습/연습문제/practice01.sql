...
select concat(first_name, ' ', last_name) as '사원이름', emp_no
from employees 
where emp_no = "10994";
...
Select gender as '성별', Count(gender) as '직원수' FROM employees
Group BY gender;

-- 문제4.
-- 현재 근무하고 있는 직원 수는 몇 명입니까? (salaries 테이블을 사용합니다.)
select count(employees.emp_no) from employees join salaries 
on employees.emp_no = salaries.emp_no
where salaries.to_date > now();

-- 문제5.
-- 부서는 총 몇 개가 있나요?
select count(dept_name) as '부서의 총 갯수' from departments;

-- 문제6.
-- 현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외) //역임: 과거에 매니저를 맡았던 사람
select  count(dept_no) from dept_manager where to_date > now();

-- 문제7.
-- 전체 부서를 출력하려고 합니다. 순서는 부서이름이 긴 순서대로 출력해 보세요.
select * from departments order by char_length(dept_name) desc;
